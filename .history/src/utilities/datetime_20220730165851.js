...
function CheckComponent(checkDate) {
    /* MOUNT COMPONENT IF TRUE */
    const expiry = dayjs(checkDate, "MMMM DD YYYY hh:mm A").tz("Asia/Manila");
    const start = dayjs(checkDate, "MMMM DD YYYY hh:mm A").tz("Asia/Manila");
    if (dayjs().tz("Asia/Manila") > expiry) {
        return true;
    }
}
